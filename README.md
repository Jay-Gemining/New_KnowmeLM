...
*   **`Sidebar.js` (左侧边栏 - Left Sidebar):**
    ...
    *   **内容来源列表:** 当用户选定一个笔记本后，该笔记本下的所有内容来源（如 YouTube 视频摘要、文本文件摘要）会清晰地展示在此。不同类型的内容来源会以直观的图标加以区分。
...
*   **嵌入式内容添加工具:**
    *   **`YoutubeSummarizer.js` 和 `TextFileSummarizer.js`:** 这些组件现在通过左侧边栏中的按钮激活，但其用户界面会显示在主内容区，引导用户输入 URL 或上传文件。
...
2.  **添加内容来源:**
    ...
    *   处理完成后，新的来源会出现在左侧边栏的来源列表中，并默认被选中用于聊天上下文。
